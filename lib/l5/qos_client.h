 ...
 *     2.添加两个错误码 
 *          QOS_RTN_LOCAL_ERROR: api内部错误如分配空间,创建类,创建socket等等，用于与QOS_RTN_SYSERR区分，使得错误码意义更加明确
...
/*注意:!!!!!!!!
 * 1.返回的列表无负载均衡和容错策略:该方法本质是读取cl5 agent写入的一个文本文件,查找对应的CL5 SID,不会与CL5 Agent通信,所以没有容错和负载均衡
 * 2.路由丢失问题:如果业务使用该方法并自己做负载均衡和容错,请注意:因为CL5 Agent目前有自动清除冗余路由(在业务机器上,某SID 一定时间(默认配置30天)内未访问,则认为路由冗余
 *      ,判断标准是:业务是否有在一定时间(默认配置30天)内通过ApiGetRoute等接口获取过路由。因为此方法没有与CL5 Agent通信,不会主动拉取,所以业务机器上该SID的访问关系丢失
 *    针对路由丢失问题,目前得到了部分解决:
